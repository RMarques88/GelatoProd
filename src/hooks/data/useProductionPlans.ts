...
export function useProductionPlans(
  ...
  const fromKey = useMemo(() => options.from?.toISOString() ?? 'from', [options.from]);
  const toKey = useMemo(() => options.to?.toISOString() ?? 'to', [options.to]);
  ...
  const subscribe = useCallback(
    ...
    [options.from, options.includeArchived, options.limit, options.status, options.to],
  );
  ...
  >({
    ...
    dependencies: [statusKey, fromKey, toKey, options.includeArchived, options.limit],
  });
  ...
  const handleUpdateStatus = useCallback(
    async (planId: string, status: ProductionStatus) => {
      let snapshot: ProductionPlan | undefined;
